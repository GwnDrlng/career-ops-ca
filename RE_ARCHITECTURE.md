...
**Cloud deploy is DONE (see the Deployment note above).** What remains is not code: (1) create the `/scan` slash command in your Slack app + set `SLACK_SIGNING_SECRET` on Vercel, then redeploy; (2) run a live end-to-end scan (`node scan-now.mjs` works today) to exercise the several 🧪 "built, live path not exercised" checklist items (judge, grounding, doc-gen, calibration grade). Local dev/daemons need `node@24` on PATH — see the Decisions section.
