...
**The whole plan is built, tested, committed, and pushed — and as of 2026-07-01 the cloud is DEPLOYED and live on Vercel.** Everything is on the `massive-updates` branch (pushed to `origin`, no PR yet).

**Deployment (2026-07-01) — DONE:** Cloud eve agent deployed to Vercel (team `career-ops`, project `cloud`, stable alias `https://cloud-xi-eight.vercel.app`). Slack wired via Vercel Connect (connector `slack/career-ops`, no manual bot token); `placeholderAuth()` swapped to `none()` in `cloud/agent/channels/eve.ts`; AI Gateway auth via Vercel OIDC (no `AI_GATEWAY_API_KEY` needed). **Two new triggers added on top of the daily cron:** (1) on-demand cloud scan from the terminal via `scan-now.mjs` (POSTs to `/eve/v1/session` with the OIDC token); (2) a real Slack `/scan` slash command via a new custom channel `cloud/agent/channels/slack-commands.ts` (Slack-signature-verified, mounted at `/slack/commands`). **Per-run spend summaries** (`🧾` tokens + notional $) now post to `#job-pipeline` from `judge.mjs`/`applier.mjs` via `opus-call.postSpendSummary()` + `token-budget.sessionSummary()`. The on-prem `watch.mjs` + `approval-consumer.mjs` daemons run untended as **launchd services** (`~/Library/LaunchAgents/com.career-ops.{watch,approval-consumer}.plist`). Control commands (`pause`/`resume`/`approve`) stay on-prem plain-text (submit authority never leaves the laptop); the leading slash is now optional since Slack swallows a real `/pause`.

**Post-deploy iteration (2026-07-02) — DONE:** the first live on-demand `/scan` runs surfaced and fixed a wave of real issues, so several formerly-🧪 paths are now exercised end-to-end. (1) **Scan persistence + reliability:** cross-run dedup moved off the in-memory placeholder to **Vercel Blob** so it survives cold starts/redeploys; portals now fetched in parallel with an always-leave-a-trace guarantee. (2) **Broader coverage:** scanner gained **Workday CXS** support (POST, paginated) on top of Greenhouse/Ashby/Lever, plus Canadian banks/insurers (BMO, CIBC, National Bank, Sun Life), more Ashby boards (Supabase, Jobber, Linear), Wiz, and ~19 new tracked companies weighted toward new verticals (robotics/AV/industrial IoT, devtools/data, new-vertical SaaS); Arctic Wolf (recent employer) removed. (3) **Grading:** industry match de-weighted to a light signal (any software / software-for-hardware role in scope, graded on transferable strengths), mirrored into both cloud grader instructions and rubric to stay comparable with on-prem. (4) **Slack shape:** reports now post as a **compact header message (via Connect) + the full Block A–G report as a `.md` file attachment** (uploaded via `SLACK_BOT_TOKEN`/`career-ops-ca`, which has `files:write`; the managed Connect connector lacks it); a `botToken` resolve bug in `post_to_slack.ts` was fixed. Instructions hardened to **forbid digest/top-N tables** and require exactly one `post_to_slack` per graded posting (a table-only row never reaches `route-tier`). (5) **On-prem now applies for real:** `watch.mjs` hands apply-eligible lanes to `applier.mjs` after routing (curated-docgen lane runs the new `pipeline/curate-docs.mjs` CV+CL generator first); `apply_enabled` flipped **`false → true`** (shadow mode off — the applier fills real forms and posts an approval request to `#job-approvals`; **nothing auto-submits**, each apply still needs a Slack `approve <token>`); `rolling_5h_block_limit_tokens` doubled `200000 → 400000` for live-pipeline headroom.

**Still external (your Slack workspace, not code):** create the `/scan` slash command in your Slack app (Request URL `https://cloud-xi-eight.vercel.app/slack/commands`) and set `SLACK_SIGNING_SECRET` on Vercel (`vercel env add`), then redeploy so the endpoint can verify requests.

Build order (all done, local): Part 6 housekeeping → Part 7 config layer (`config/guardrails.yml`, `config/blocklist.yml`) → Part 4 (doc-gen + judge) → Part 5 (token budget) → Part 2 (watcher + router) → Part 3 (applier) → Part 1 (cloud agent — typechecked/built/smoke-tested via `eve dev`, **not deployed**) → **Unit 8** (safety) → **Unit 9** (observability + change control) → **Part 6 close-out** (`test-all.mjs` section 17 + the 20-point checklist walk). All bespoke code lives in `pipeline/` + `watcher/` + `cloud/` (see "Critical files" and the Part sub-sections below).

- **Unit 8 (safety) — DONE.** Approval-delivery gate (`approval.mjs`: single-use/12h-expiry/verified-approver), kill switch (`kill-switch.mjs`: pause/resume, auto-pause, scheduled defer + auto-resume), Slack control consumer (`approval-consumer.mjs`), and `sanitize-jd.mjs` (prompt-injection filter, wired into applier/judge/grounding-check).
- **Unit 9 (observability + change control) — DONE.** 7E: audit log (`audit.mjs`), monthly + per-application cost caps, manual budget override + defer-until-next-cron, dead-letter queue, grading calibration spot-check. 7F: config-change logging (`config-guard.mjs`), prompt/rubric versioning (`prompt-version.mjs` + `config/prompt-versions.yml`, stamped into judge-history + calibration-log), accepted-risk register (`RISK_REGISTER.md`).
- **Part 7 (A–F governance/safety/guardrails) — COMPLETE.**

**Test status:** re-architecture guardrails are covered by `test-all.mjs` **section 17** (26 checks, green) plus the per-unit behavioral suites cited in the Part sub-sections. Section 17 caught + fixed a real regression (unguarded CLI in `prompt-version.mjs`/`config-guard.mjs`/`token-budget.mjs`). The full `test-all` suite also shows **7 pre-existing failures unrelated to this work** — upstream system files absent from this checkout (`validate-portals.mjs`, `updater-migration-tests.mjs`, `modes/interview.md`) + data-quality errors in the gitignored `data/applications.md`. These are **accepted and will NOT be fixed by syncing upstream** (this fork is never updated from the original repo).

**Cloud deploy is DONE (see the Deployment + Post-deploy notes above).** Live on-demand scans have now run end-to-end (scan → grade → per-posting Slack report → on-prem watcher → route → applier fill → `#job-approvals`), so the scan/grade/report/route/fill paths are exercised. What remains is not code: (1) create the `/scan` slash command in your Slack app + set `SLACK_SIGNING_SECRET` on Vercel, then redeploy; (2) exercise the still-🧪 on-prem Opus paths that cost real tokens (judge, grounding-check, curated doc-gen quality, calibration grade) against a real high-scoring posting; (3) a real form submit once you're ready to let a filled+approved apply through (`apply_enabled` is now `true`, but no live submit has been driven yet). Local dev/daemons need `node@24` on PATH — see the Decisions section.
...
- **Cost model — two DIFFERENT wallets (important):** the two halves of the pipeline bill to different places, and only one is real money.
  - **Cloud grading (scan + grade, via Vercel AI Gateway) = REAL pay-as-you-go money on the Vercel account** (requires a credit card on file — the AI Gateway refuses requests without one; discovered 2026-07-01 when the first live `/scan` returned `GatewayInternalServerError: AI Gateway requires a valid credit card on file`). Scanning itself is zero-token (direct portal APIs), but each new posting is graded by Sonnet and that bills Vercel. Cap it with a **spend limit in the Vercel AI Gateway dashboard** (`vercel.com/[team]/~/ai`) — the cloud equivalent of the on-prem budget caps.
  - **On-prem Opus work (judge, grounding-check, applier drafts) = notional, NOT a real bill** — it runs on the user's **Claude Pro subscription** (flat CAD fee), so the `total_cost_usd` that `claude -p` reports and everything derived from it (`config/guardrails.yml` `cost.*`, the `🧾` per-run spend summaries) is an **API-equivalent usage proxy**, not money charged. Its real constraint is the plan's 5h/weekly usage windows, not dollars.
  - **Takeaway:** the `cost.*`/`token_budget.*` guardrails and `🧾` summaries govern the *notional* on-prem side; the *real* spend to actually watch is the **Vercel AI Gateway** bill, capped in Vercel's dashboard, not in `guardrails.yml`.
...
| **2.1 – 3.6** | **Generic CV** (the pre-generated generic PDF at `config/profile.yml`'s `cv.generic_pdf`, always PDF) → **Applier** fills the online form and attaches that PDF to any resume-upload field. If the form has **curated questions** ("why do you want to work for X?", "tell us about yourself", "describe a past situation/example") → **flag manual** in Slack. Else → post filled form to Slack for **1-click approve** → submit. |
...
- `cloud/agent/tools/scan_portals.ts` (inside the scanner subagent) — direct Greenhouse/Ashby/Lever API fetches, **plus Workday CXS endpoints** (POST, paginated) for shops on Workday (Canadian banks/insurers: BMO, CIBC, National Bank, Sun Life). Portals are fetched **in parallel** with an always-leave-a-trace guarantee (a portal failing doesn't silently drop the run). Reads `cloud/agent/lib/portals-config.ts` (a **generated TS module**, not a runtime YAML read — eve's `lib/` discovery only accepts real code modules, and this sidesteps uncertainty about whether Vercel's build tracing would bundle an out-of-tree file read). Regenerate that module by hand when the root `portals.yml` changes. Cross-run dedup is `cloud/agent/lib/dedup.ts` — now **Vercel Blob-backed** (persists across schedule invocations, cold starts, and redeploys); the earlier in-memory placeholder is gone, so the KV/Upstash/Postgres decision is resolved (Blob).
...
- `cloud/agent/tools/post_to_slack.ts` (root tool) — posts a **compact header message** (via the Connect-brokered bot token from `connectSlackCredentials("slack/career-ops")`) **plus the full Block A–G report as a `.md` file attachment**, so `#job-pipeline` stays readable and the on-prem watcher reconstructs the complete report from the attached file. The **file upload routes through `SLACK_BOT_TOKEN` (the `career-ops-ca` bot, which has `files:write`)** because the managed Connect connector's scopes are fixed by Vercel's Slack app and lack `files:write`; channel messaging stays on Connect. Live-verified during the 2026-07-02 scans (which also caught + fixed a bug where the async `botToken` factory was stringified into the `Authorization` header instead of being resolved). One `post_to_slack` per graded posting is mandatory — digest/top-N tables are forbidden (see `instructions.md`).
- `cloud/agent/subagents/grader/skills/grading-rubric.md` — ported 1–5 weighted rubric + archetypes + legitimacy tiers, adapted for cloud (no Playwright/page-snapshot access, so legitimacy defaults more conservatively to "Proceed with Caution"). **Industry match is a light signal, not a filter** (2026-07-02): any software / software-for-hardware role is in scope, graded on transferable strengths (product craft, P&L, 0-to-1, portfolio, strategy, GTM) rather than docked for domain unfamiliarity; prior-domain depth (cyber/fintech) stays a bonus, never a penalty when absent. Mirrored into `grader/instructions.md`'s embedded profile so cloud and on-prem grades stay comparable.
...
- `watcher/watch.mjs` — polls Slack via bot token (Keychain, service `career-ops-slack-bot-token`), prefers the attached `.md` report file over the compact header message, writes report into `reports/{###}-{slug}-{date}.md` via `reserve-report-num.mjs`, invokes `pipeline/route-tier.mjs`. Checkpoints last-seen Slack ts in `watcher/.checkpoint.json` (gitignored). **After routing, it hands apply-eligible lanes to `pipeline/applier.mjs`** (2026-07-02) so a routed posting reaches a filled form + `#job-approvals` request instead of stopping at the tracker; the applier re-runs its own preflight gates so the handoff is safe. For the **curated-docgen lane (≥3.7) only**, `watch.mjs` first runs `pipeline/curate-docs.mjs` (CV+CL generation) and posts the doc paths to `#job-pipeline` before the applier hand-off; the generic-apply lane goes straight to the applier.
...
- Non-curated: sensitive fields fill only from `pipeline/vault.mjs` (Keychain-backed, never invents a value); everything else drafted via a separate Opus 4.8 call (`pipeline/opus-call.mjs`); fills via Playwright, screenshots, and **stops** at a Slack approval gate. **Salary-expectation** (2026-07-02) fills with the **midpoint of the comp band the posting discloses** (parsed from the report's Comp section, clamped to a plausible salary window so proof-point figures don't leak in); when the posting discloses no band it falls back to the personal default in the local Keychain vault — no comp number is baked into the committed file. Applied in both the fill and post-approval submit phases. Also hardened (2026-07-02) to parse Slack-wrapped URLs and use robust fill selectors so it actually reaches ATS forms. As of unit 8 the filled form posts a single-use/12h-expiry/verified-approver approval token to `#job-approvals` (`pipeline/approval.mjs`); the fill phase still never clicks submit. Submission happens only on `applier.mjs --submit` after a verified approver replies `approve <token>` — and even then only if `apply_enabled: true`.
...
- **Generic CV:** the pre-generated generic PDF (path from `config/profile.yml`'s `cv.generic_pdf`, user-layer/gitignored so no personal filename lives in the repo) built from `cv.md` via `generate-pdf.mjs` (unchanged, reused as planned). The applier reads that path via `resolveGenericCvPath()` and attaches it to resume-upload fields; if unset/missing, the upload field is flagged for manual handling.
- **Curated CV + CL:** both `.docx`. `modes/cover.md` (new, interactive mode) + `generate-cover-letter.mjs` (new, root — generalized content-JSON-driven generator) replace the old plan of "generalizing" `gen-cl-docx.mjs`, which was left in place untouched since it holds real historical per-company letters, not dead code. Discovery note: `modes/pdf.md` already referenced both files by name before this build — they were a pre-existing dangling dependency, not a new invention.
  - **Orchestrator: `pipeline/curate-docs.mjs`** (new, 2026-07-02) — the step that actually produces the curated docs in the automated lane (previously the ≥3.7 lane promised a tailored CV+CL but nothing generated them; `watch.mjs` handed straight to `applier.mjs`, which only drafts inline form answers). It parses the report, reads `cv.md` + profile identity, makes **one token-gated Opus call** (`opus-call.mjs`, grounded in `cv.md`, no invented metrics, no em dashes) returning resume + cover-letter content JSON, and renders both via `generate-docx.mjs` / `generate-cover-letter.mjs`. The report body is run through `sanitizeJd()` first; a budget halt or LLM failure is a clean refusal, not a crash. Wired into `watch.mjs` for the curated-docgen lane only, before the applier hand-off.
...
- `rolling_5h_block_limit_tokens` / `weekly_limit_tokens` in `config/guardrails.yml` ship as `null` in the template (real token allowance isn't API-discoverable), in which case the guardrail reports "not configured" and never halts. **For this deployment they're now set to working ESTIMATES for Claude Pro** — `rolling_5h_block_limit_tokens: 400000` (doubled from 200000 on 2026-07-02 for live-pipeline headroom) and `weekly_limit_tokens: 1000000` — tune both from real usage in `data/token-usage.tsv`.
...
### Part 6 — Remove update-on-startup + housekeeping — **DONE.** Update-check auto-trigger removed from `CLAUDE.md`/`AGENTS.md` (now user-triggered only, with previously-silent statuses now reporting back). `modes/ru/_shared.md` needed no change (different, still-accurate statement). `GEMINI.md` is a stub, nothing to remove.
- Delete the **"## Update Check"** block from `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `modes/ru/_shared.md`. Keep manual `update-system.mjs`. Leave cold-start `doctor.mjs` onboarding intact. **DONE.**
- `test-all.mjs` **section 17** (new) covers: report-format parity (Machine Summary parseable), tier thresholds (2.0/2.1/3.6/3.7), curated-question classifier, judge 90%-gate + retry-cap config parity, token-budget 4-ceiling structure, rubric parity (cloud vs local), plus the unit 8-9 guardrails (sanitize-jd, calibrate drift, config-guard, prompt-version, module-export sanity) and a `pipeline/`+`watcher/` syntax sweep. **26 checks, green.** The 20-point checklist below is walked + annotated with per-item status. (Judge retry-cap + history-logging *behavior* is 🧪 — logic + config tested, live Opus run not exercised.)
...
- **Shadow mode:** built + tested, now **switched off for live use.** `applier.mjs` hard-refuses (no browser launch at all) when `apply_enabled: false`, and pause takes precedence over the shadow-mode check — both verified by test. As of 2026-07-02 `config/guardrails.yml` sets **`apply_enabled: true`** so the applier fills real forms and posts an approval request to `#job-approvals`; the shadow-mode refusal path remains available (flip back to `false` to re-enter it). **Nothing auto-submits regardless** — every apply still requires a Slack `approve <token>`.
- **Post-approval submit:** `applier.mjs --submit --token <t> --approver <id>` (driven by the consumer) re-verifies+burns the token, re-runs every pre-submit gate + liveness, re-fills, clicks submit, records `Applied` to the tracker via the TSV+merge path, and posts a confirmation. Gated by `apply_enabled`, now `true` (2026-07-02) — so a filled + Slack-approved form can submit live; no live submit has been driven yet.
...
- **Accepted-risk register:** DONE. `RISK_REGISTER.md` — 10 tracked risks (post-hoc cost accounting, unexercised live-submit path, cloud dedup persistence, heuristic sanitize-jd, Connect post split-token, user-id-based Slack auth, estimated (not plan-verified) ceilings, model-vs-artifact versioning, poll-driven auto-resume, generic form selectors), each with severity/likelihood/posture, mitigation, and a concrete **revisit trigger** so "accepted" never silently becomes "forgotten." Refreshed 2026-07-02 as the deploy resolved several to `Mitigated` (R2/R3/R5/R7).
...
- New: `cloud/` (eve project), `watcher/watch.mjs`, `pipeline/route-tier.mjs`, `pipeline/applier.mjs`, `pipeline/curate-docs.mjs` (curated CV+CL generation orchestrator for the ≥3.7 lane), `pipeline/judge.mjs` (rubric is inline in the prompt, not a separate `judge-rubric.md` file), `pipeline/token-budget.mjs`, `generate-cover-letter.mjs` (root), `modes/cover.md`, `reserve-report-num.mjs` (root).
- New (guardrails — unit 8): `config/guardrails.yml`, `config/blocklist.yml`, `pipeline/vault.mjs` (Keychain), `pipeline/grounding-check.mjs`, `pipeline/sanitize-jd.mjs`, `pipeline/kill-switch.mjs`, `pipeline/approval.mjs`, `watcher/approval-consumer.mjs` (Slack `/pause` `/resume` + `approve <token>` + `budget-override`/`budget-defer` handler).
- New (observability + change control — unit 9): `pipeline/audit.mjs` (append-only ledger), `pipeline/dead-letter.mjs` (poison-message queue), `pipeline/budget-override.mjs` (manual override + defer), `pipeline/calibrate.mjs` (grading calibration spot-check), `pipeline/config-guard.mjs` (guardrails-change logging), `pipeline/prompt-version.mjs` + `config/prompt-versions.yml` (prompt/rubric versioning), `RISK_REGISTER.md` (accepted-risk register). Also: `test-all.mjs` **section 17** (added, not new file).
- Data (all live, gitignored): `data/judge-history.tsv`, `data/token-usage.tsv` (now with `cost_usd` + `job_id` cols), `data/audit-log.tsv`, `data/pipeline-state.json` (kill-switch, incl. scheduled `pausedUntil`), `data/approvals.json`, `data/budget-overrides.json`, `data/dead-letter.tsv` + `data/dead-letter-retries.json`, `data/calibration-log.tsv`, `data/guardrails-snapshot.json`, `data/prompt-hashes.json`.
...
1. ✅/☁️ **Cloud scan:** deployed and run live (2026-07-02) across Greenhouse/Ashby/Lever/Workday with Blob-backed dedup; a formal parity diff vs `node scan.mjs --dry-run` is still worth doing.
...
3. ✅ **Slack round-trip:** live — cloud posts compact header + `.md` report attachment to `#job-pipeline` (Connect for messaging, `SLACK_BOT_TOKEN` for the file upload); the on-prem watcher ingests the attached report.
...
6. 🧪 **Doc formats:** generic PDF / curated `.docx` paths built; curated lane now wired end-to-end via `pipeline/curate-docs.mjs` (`watch.mjs` runs it before the applier hand-off), but the live Opus doc-gen call is not yet exercised.
...
17. ✅ **Shadow mode:** `apply_enabled:false` → zero fills/submits, refusal before browser launch (tested). Now set `true` (2026-07-02) for live fill; auto-submit is still impossible without a Slack `approve <token>`.
