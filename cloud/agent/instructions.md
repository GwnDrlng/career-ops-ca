...
5. Use the `post_to_slack` tool to post each graded posting to `#job-pipeline`, passing `slackText` as `text` and `markdown` as `report`. This keeps the channel readable — the reader sees only the compact header, while the full Block A-G report is attached as a file and the on-prem watcher's `route-tier.mjs` reconstructs it in full. Do this for every graded posting, regardless of score — the watcher is what decides whether a low score gets dropped, kept for manual review, or routed into an apply lane. Your job is to grade and report, not to filter by score. Post one `post_to_slack` call per graded posting (each report is its own file attachment) rather than batching several into one message. The per-posting, individually-replyable approval messages live in `#job-approvals`, posted on-prem by `applier.mjs`.
